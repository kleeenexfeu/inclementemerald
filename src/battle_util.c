...
void HandleAction_ActionFinished(void)
{
    ...
    *(gBattleStruct->monToSwitchIntoId + gBattlerByTurnOrder[gCurrentTurnActionNumber]) = 6;
    ...
    for (i = gCurrentTurnActionNumber; i < gBattlersCount - 1 - gCurrentTurnActionNumber; i++)
...
// taken action. It's been previously increased, which we want to not recalculate the turn order of the mon that just finished its action
    ...
    {
        for (j = i + 1; j < gBattlersCount - gCurrentTurnActionNumber; j++)
        {
...
u8 IsMonDisobedient(void)
{
    ...
    if (1) // mew/deoxys obedience crap
    {
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    case HOLD_EFFECT_SOUL_DEW:
        ...
        #else
        if ((gBattleMons[battlerAtk].species == SPECIES_LATIAS || gBattleMons[battlerAtk].species == SPECIES_LATIOS) && IS_MOVE_SPECIAL(move))
        #endif
            MulModifier(&modifier, holdEffectModifier);
        break;
...
static u32 CalcDefenseStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    case HOLD_EFFECT_SOUL_DEW:
        if ((gBattleMons[battlerDef].species == SPECIES_LATIAS || gBattleMons[battlerDef].species == SPECIES_LATIOS) && !usesDefStat)
            MulModifier(&modifier, UQ_4_12(1.5));
        break;
#endif
    }
